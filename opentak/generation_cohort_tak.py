...
class GenerateCohortTAK:
    def __init__(self, nb_patients=500, nb_days_end=365, random_state: int | None = None):
    ...
    def initialisation_dataframe(self, treatment_name="A", dose_mean=30, dose_std=10):
        ...
        dose_real = self.rng.normal(dose_mean, dose_std, size=(self.nb_patients, nb_deliveries_max)).astype(int)
    ...
    def add_switch_linear(
        ...
        distrib_nbdays_switch = self.rng.integers(start_period_switch, end_period_switch, size=self.nb_patients)

        # Applies the switch to proportion_of_cohort and adds the lines to the dataframe.
        return self._add_switch(treatment_name, distrib_nbdays_switch, proportion_of_cohort)

    def add_switch_gaussien(self, treatment_name, mean=None, std=None, proportion_of_cohort=1):
        # ruff: noqa: D205
    ...
    def add_drug_holidays(
        ...
        distrib_nbdays_start_dh = self.rng.integers(start_dh_min, start_dh_max, size=self.nb_patients)
        ...
        self.base["nbdays_start_dh"] = np.repeat(distrib_nbdays_start_dh, self.nb_rows_per_patient)
        self.base["nbdays_end_dh"] = np.repeat(distrib_nbdays_end_dh, self.nb_rows_per_patient)
        self.base = self.base[
        ...
        # updating the number of lines per patient
    ...
    def _add_switch(self, treatment_name, distrib_nbdays_switch, proportion_of_cohort):
        ...
        self.base.loc[self.base["switch"].le(self.base["TIMESTAMP"]), "EVT"] = treatment_name
    ...
    def add_in_out(self, proba_death=0):
        ...
        base_out["TIMESTAMP"] = self.rng.integers(self.nb_days_end + 1, size=len(base_out))
